...
exports.run = async (client, msg, [money, ...search]) => {
    ...
    return msg.prompt(`Dear ${msg.author}, you're going to pay ${money}${shiny(msg)} to ${user.username}, do you accept?`)
        .then(async () => {
            await user.profile.add(money);
            await msg.author.profile.use(money);
            return msg.alert(`Dear ${msg.author}, you have just paid ${money}${shiny(msg)} to **${user.username}**`);
        })
