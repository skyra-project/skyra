...
exports.run = async (client, msg, [query]) => {
  ...
  const response = await snekfetch.get(`https://maps.googleapis.com/maps/api/geocode/json?address=${locationURI}&key=${GOOGLE_MAP_API}`)
    .then(d => JSON.parse(d.text));
  ...
  const res = await snekfetch.get(`https://api.darksky.net/forecast/${WEATHER_API}/${params}?exclude=minutely,hourly,flags&units=si`)
    .then(d => JSON.parse(d.text));
  ...
  return this.draw(client, msg, { geocodelocation, state, city, condition, icon, chanceofrain, temperature, humidity });
};
...
exports.handleNotOK = (msg, status) => {
