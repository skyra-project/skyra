...
class Resolver {
    ...
    async user(user) {
        ...
        if (typeof user === 'string' && this.constructor.regex.userOrMember.test(user)) {
            return this.client.fetchUser(this.constructor.regex.userOrMember.exec(user)[1]).catch(() => null);
        }
        return null;
    }
    ...
    async member(member, guild) {
        ...
        if (typeof member === 'string' && this.constructor.regex.userOrMember.test(member)) {
            const user = await this.client.fetchUser(this.constructor.regex.userOrMember.exec(member)[1]).catch(() => null);
    ...
    async advMember(query, msg) {
        ...
        return user ? msg.guild.fetchMember(user).catch(() => null) : null;
    }
    ...
    async boolean(bool) {
        ...
        if (['1', 'true', '+', 't', 'yes', 'y', 'on'].includes(String(bool).toLowerCase())) return true;
        if (['0', 'false', '-', 'f', 'no', 'n', 'off'].includes(String(bool).toLowerCase())) return false;
        return null;
    }
