...
class ArgResolver extends Resolver {
	...
	provider(arg, currentUsage, possible, repeat, msg) {
		const provider = this.client.providers.get(arg);
		if (provider) return Promise.resolve(provider);
		if (currentUsage.type === 'optional' && !repeat) return Promise.resolve(null);
		return Promise.reject(msg.language.get('RESOLVER_INVALID_PIECE', currentUsage.possibles[possible].name, 'provider'));
	}
