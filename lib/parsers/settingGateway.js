...
class SettingGateway {
    ...
    async update(guild, object) {
        ...
        for (const [key, value] of Object.entries(object)) {
            ...
            values[key] = {};
            for (const [subkey, subvalue] of Object.entries(value)) {
                if (subkey in schema === false) throw `The key ${key}→${subkey} does not exist in the current schema.`;
    ...
    async updateArray(guild, type, key, subkey, data) {
        ...
        if (this.schema[key][subkey].array !== true) throw `The key ${key}→${subkey} is not an Array.`;
