...
class Gateway {
	 ...
	 * @param {string} input An object containing a id property, like discord.js objects, or a string.
	 * @returns {Promise<void>}
	 */
    async sync(input) {
    ...
    async reset(target, key, avoidUnconfigurable = false) {
        ...
        const { parsed, settings } = this._reset(target, key, guild, { path, route });

        await guild.settings.update(settings);
        return { value: parsed, path };
    }

    _reset(target, key, guild, { path, route }) {
