...
class Gateway {
    ...
    async createEntry(input, data = this.defaults) {
        const target = this.validate(input).then(output => output && output.id ? output.id : output);
    ...
    async reset(target, key, avoidUnconfigurable = false) {
        ...
        target = this.validate(target).then(output => output && output.id ? output.id : output);
