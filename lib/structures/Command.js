...
class Command {
    ...
    static buildExtendedHelp(name, extend) {
        ...
        if (extend.EXP_USAGE) {
            const EXP_USAGE = ['⚙ | ***Explained usage***', `Skyra, ${name} ${extend.ARGUMENTS}`];
            for (const [argument, description] of extend.EXP_USAGE) EXP_USAGE.push(`→ **${argument}**: ${description}`);
            output.push(EXP_USAGE.join('\n'), '');
        }

        if (extend.EXAMPLES) {
            const EXAMPLES = ['🔗 | ***Examples***'];
            for (const example of extend.EXAMPLES) EXAMPLES.push(`→ Skyra, ${name} *${example}*`);
            output.push(EXAMPLES.join('\n'));
        } else {
            output.push('🔗 | ***Examples***', `→ Skyra, ${name}`);
        }
