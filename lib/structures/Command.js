...
class Command {
	...
	constructor(client, dir, file, options = {}) {
		...
		/**
		 * Whether the command should run only on guilds.
		 * @type {boolean}
		 */
		 ...
		 * @type {string[]}
		 */
		this.aliases = typeof options.aliases === 'object' ? options.aliases : [];

		/**
		 * The required permLevel to run this command
		 * @type {number}
		 */
		this.permLevel = typeof options.permLevel === 'number' ? options.permLevel : 0;
		...
		/**
		 * Whether the command should run in a configured spam channel.
		 * @type {boolean}
		 */
		this.spam = typeof options.spam === 'boolean' ? options.spam : false;

		/**
		 * The name of the command
		 * @type {string}
		 */
		this.name = typeof options.name === 'string' ? options.name : file[file.length - 1].slice(0, -3);
		 ...
		 * @type {?string}
		 */
		this.usageDelim = typeof options.usageDelim === 'string' ? options.usageDelim : undefined;

		/**
		 * Whether to use quoted string support for this command or not
		 * @type {boolean}
		 */
		this.quotedStringSupport = typeof options.quotedStringSupport === 'boolean' ? options.quotedStringSupport : false;
	...
	static handleError(err) {
		if (err && err instanceof DiscordAPIError) {
			const tempError = new Error(err.message);
			tempError.name = err.name;
			err.stack = tempError.stack;
		}
		throw err;
	}
...
require('./interfaces/Piece').applyToClass(Command);
