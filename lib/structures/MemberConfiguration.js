...
class MemberConfiguration {
	 ...
	 */
	async sync() {
		const [data] = await this.provider.db.table('localScores').getAll([this.guildID, this.userID], { index: 'guild_user' }).limit(1).catch(() => []);
	...
	async update(amount) {
		...
		await (this.UUID
			? this.provider.db.table('localScores').get(this.UUID).update({ count: amount | 0 })
			: this.provider.db.table('localScores').insert({ guildID: this.guildID, userID: this.userID, count: amount | 0 })
	...
	async destroy() {
		if (this.UUID) {
			await this.provider.db.table('localScores').get(this.UUID).delete();
			this.UUID = null;
		}
		this.count = 0;
	}
