...
class MemberConfiguration {
	...
	async sync() {
		const data = await this._runQuery('runOne',
			`SELECT * FROM "localScores" WHERE "id" = '${this.memberID}' AND "guild" = '${this.guildID}' LIMIT 1;`).catch(() => null);
	...
	async update(amount) {
		...
		await this._runQuery('run', this._existsInDB
			? `UPDATE "localScores" SET "count" = ${amount | 0} WHERE "id" = '${this.memberID}' AND "guild" = '${this.guildID}'`
	...
	_runQuery(type, query) {
		...
		this._syncStatus
			.then(() => { this._syncStatus = null; })
			.catch(error => { throw `Failed query ${query}\n${error.stack || error}`; });
