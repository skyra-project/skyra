...
const { getImage, deIdiotify } = require('../util/util');
...
module.exports = class StarboardMessage {
	...
	async updateMessage() {
		...
		if (this.starMessage) {
			try {
				await this.starMessage.edit(content, { embed: this.embed }).catch(deIdiotify);
			} catch (error) {
	...
	async _sync() {
		...
		if (data) {
			...
			if (!this.starMessage && this.stars < this.manager.minimum) await channel.send(this.embed);
		}
