...
module.exports = class Moderation {
	...
	exists() {
		return RethinkDB.has('moderation', this.id);
	}

	create() {
		return RethinkDB.create('moderation', {
			id: this.id,
			cases: []
		});
	}
	...
	getCases(id) {
		if (typeof id === 'number')
			return RethinkDB.getFromArrayByIndex('moderation', this.id, 'cases', id);

		return RethinkDB.get('moderation', this.id).then(doc => doc ? doc.cases : this.ensureModule().then(() => []));
	}
	...
	async pushCase(data) {
		...
		await RethinkDB.append('moderation', this.id, 'cases', data);
	...
	updateCase(index, doc) {
		return RethinkDB.updateArrayByIndex('moderation', this.id, 'cases', index, doc);
	}
	...
	async destroy() {
		...
		return RethinkDB.delete('moderation', this.id);
	}
