...
class Fetch {
	...
	resolveRole(query, guild) {
		...
		if (typeof query === 'string' && REGEXP.role.test(query)) return guild.roles.get(REGEXP.role.exec(query)[1]);
		return null;
	}
	...
	resolveChannel(query, guild) {
		...
		if (typeof query === 'string' && REGEXP.channel.test(query)) return guild.channels.get(REGEXP.channel.exec(query)[1]);
		return null;
	}
	...
	resolveUser(query, guild) {
		...
		if (typeof query === 'string') {
			if (REGEXP.userOrMember.test(query)) return this.client.users.fetch(REGEXP.userOrMember.exec(query)[1]).catch(() => null);
