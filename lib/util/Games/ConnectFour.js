...
module.exports = class ConnectFour {
	...
	async run(message) {
		...
		this.message = await message.edit(this.language.get('SYSTEM_LOADING')).catch(deIdiotify);
		...
		while (this.running) { // eslint-disable-line
			...
			try {
				...
				if (error === RESPONSES.FULL_LINE) {
					...
					await this.message.edit(this.language.get('COMMAND_C4_GAME_DRAW', this.renderTable())).catch(deIdiotify);
					...
					await this.message.edit(this.language.get('COMMAND_C4_GAME_TIMEOUT')).catch(deIdiotify);
					break;
