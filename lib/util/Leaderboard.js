	...
	*/
	...
	_syncMembers(guild) {
		const promise = new Promise(async (resolve) => {
			...
			const data = await r.table('localScores').filter({ guildID: guild }).limit(LIMITS.MEMBERS).orderBy(r.desc('count'));
	...
	_syncUsers() {
		const promise = new Promise(async (resolve) => {
			// Get the sorted data from the db
			const r = this.client.providers.default.db;
			const data = await r.table('users').limit(LIMITS.GLOBAL).orderBy(r.desc('points'));
