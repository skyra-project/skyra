	...
	*/
	...
	_syncMembers(guild) {
		const promise = new Promise(async (resolve) => {
			...
			const data = await r.table('localScores').filter({ guildID: guild }).orderBy(r.desc('count'));
			...
			let i = 1;
			for (const entry of data) store.set(entry.userID, { name: null, points: entry.count, position: i++ });
	...
	_syncUsers() {
		const promise = new Promise(async (resolve) => {
			// Get the sorted data from the db
			const r = this.client.providers.default.db;
			const data = await r.table('users').orderBy(r.desc('points'));

			// Get the store and initialize the position number, then save all entries
			this.users.clear();
			let i = 1;
			for (const entry of data) this.users.set(entry.id, { name: null, points: entry.points, position: i++ });
