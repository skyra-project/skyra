...
class Moderation {
	...
	async updateCase(guild, data) {
		await (data.id
			? this.r.table(TABLENAME).get(data.id)
			: this.r.table(TABLENAME)
	...
	async getCases(guild, data = {}) {
		const rows = await this.r.table(TABLENAME)
			.filter({ [Moderation.schemaKeys.GUILD]: this._checkGuild(guild), ...data }).orderBy(this.r.asc(Moderation.schemaKeys.CASE));
