...
class Util {
	...
	static async parseModlog(client, guild, modlog) {
		const [moderator, user] = await Promise.all([
			client.users.fetch(modlog[schemaKeys.MODERATOR]).catch(() => ({ tag: 'Unknown', id: modlog[schemaKeys.MODERATOR] })),
			client.users.fetch(modlog[schemaKeys.USER]).catch(() => ({ tag: 'Unknown', id: modlog[schemaKeys.USER] }))
		]);
