...
module.exports = class extends Monitor {
	...
	async run(msg) {
		...
		if (amount >= msg.guild.configs.selfmod.nmsthreshold) {
			await msg.guild.members.ban(msg.author.id, { days: 0, reason: msg.language.get('CONST_MONITOR_NMS') }).catch(this.handleError);
			await msg.sendMessage(msg.language.get('MONITOR_NMS_MESSAGE', msg.author)).catch(this.handleError);
