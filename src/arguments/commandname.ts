...
export class UserArgument extends Argument<Command> {
	public async run(parameter: string, { message, minimum }: ArgumentContext) {
		...
		const command = await new FuzzySearch(commands, (command) => command.name).run(message, parameter, minimum);
