...
export default class extends Argument {
	...
	public async run(arg: string, possible: Possible, message: KlasaMessage, filter?: (entry: Role) => boolean): Promise<Role> {
		if (!arg) throw message.language.get(LanguageKeys.Resolvers.InvalidRoleName, { name: possible.name });
		...
		throw message.language.get(LanguageKeys.Resolvers.InvalidRoleName, { name: possible.name });
	}
