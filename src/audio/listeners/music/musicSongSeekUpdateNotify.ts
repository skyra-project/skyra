...
export class UserAudioListener extends AudioListener {
	public async run(acknowledgeable: MessageAcknowledgeable, time: number) {
		const content = await resolveKey(acknowledgeable, LanguageKeys.Commands.Music.SeekSuccess, { time });
