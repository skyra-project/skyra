...
export default class extends SkyraCommand {
	...
	public run(message: KlasaMessage, [amountOrDice = 1]: [number | string | undefined]) {
		return message.sendLocale('COMMAND_DICE_OUTPUT', [
			{
	...
	private roll(message: KlasaMessage, pattern: string | number) {
		...
		if (typeof pattern === 'number') {
			if (!isNumber(pattern) || pattern <= 0) throw message.language.get('RESOLVER_INVALID_INT', { name: 'dice' });
			amount = pattern;
			dice = 6;
		} else {
			const results = this.kDice20RegExp.exec(pattern);
			if (results === null) throw message.language.get('COMMAND_DICE_ROLLS_ERROR');
			amount = typeof results[1] === 'undefined' ? 1 : Number(results[1]);
			dice = Number(results[2]);

			if (amount <= 0 || amount > 1024) throw message.language.get('COMMAND_DICE_ROLLS_ERROR');
			if (dice < 3 || dice > 1024) throw message.language.get('COMMAND_DICE_SIDES_ERROR');
