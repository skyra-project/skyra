...
export default class extends SkyraCommand {
	...
	public constructor(store: CommandStore, file: string[], directory: string) {
		...
		this.createCustomResolver('cointype', (arg, possible, message) => {
			...
			const face = message.language.tget('COMMAND_COINFLIP_COINNAMES').find(coin => coin.toLowerCase() === lArg);
			if (!face) throw message.language.tget('COMMAND_COINFLIP_INVALID_COINNAME', cleanMentions(message.guild!, arg));
			return face;
		});
	}
	...
	public async run(message: KlasaMessage, [bet, guess]: [string, string]) {
		...
		const updatedBalance = coinNames[result] === guess ? money + wager : money - wager;

		await message.author.settings.update(UserSettings.Money, updatedBalance);
		return message.sendEmbed(new MessageEmbed()
			.setColor(getColor(message))
			.setTitle(message.language.tget(coinNames[result] === guess ? 'COMMAND_COINFLIP_WIN_TITLE' : 'COMMAND_COINFLIP_LOSE_TITLE'))
			.setDescription(message.language.tget(coinNames[result] === guess ? 'COMMAND_COINFLIP_WIN_DESCRIPTION' : 'COMMAND_COINFLIP_LOSE_DESCRIPTION', coinNames[result], wager))
