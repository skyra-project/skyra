...
export class UserCommand extends SkyraCommand {
	...
	public async remove(message: GuildMessage, args: SkyraCommand.Args) {
		...
		await message.guild.writeSettings((settings) => {
			...
			if (index === -1) {
				throw args.t(LanguageKeys.Commands.Management.ManageCommandAutoDeleteRemoveNotSet, { channel: channel.toString() });
			}
	...
	public async show(message: GuildMessage, args: SkyraCommand.Args) {
		...
		if (!commandAutoDelete.length) throw args.t(LanguageKeys.Commands.Management.ManageCommandAutoDeleteShowEmpty);
		...
		if (!list.length) throw args.t(LanguageKeys.Commands.Management.ManageCommandAutoDeleteShowEmpty);
		return message.send(
