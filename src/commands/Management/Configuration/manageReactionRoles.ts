...
export class UserCommand extends SkyraCommand {
	public async add(message: GuildMessage, args: SkyraCommand.Args) {
		...
		if (!reaction) throw args.t(LanguageKeys.Commands.Management.ManageReactionRolesAddMissing);
	...
	public async remove(message: GuildMessage, args: SkyraCommand.Args) {
		...
		const reactionRole = await message.guild.writeSettings((settings) => {
			...
			if (reactionRoleIndex === -1) throw args.t(LanguageKeys.Commands.Management.ManageReactionRolesRemoveNotExists);
	...
	public async reset(message: GuildMessage, args: SkyraCommand.Args) {
		await message.guild.writeSettings((settings) => {
			...
			if (reactionRoles.length === 0) {
				throw args.t(LanguageKeys.Commands.Management.ManageReactionRolesResetEmpty);
			}
	...
	public async show(message: GuildMessage, args: SkyraCommand.Args) {
		...
		if (reactionRoles.length === 0) {
			throw args.t(LanguageKeys.Commands.Management.ManageReactionRolesShowEmpty);
		}
