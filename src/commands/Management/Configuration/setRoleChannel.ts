...
export default class extends SkyraCommand {
	...
	public async run(message: KlasaMessage, [channel]: [TextChannel | 'here']) {
		...
		if (message.guild.settings.get('channels.roles') === channel.id) throw message.language.get('CONFIGURATION_EQUALS');
		await message.guild.settings.update([['channels.roles', channel], ['roles.messageReaction', null]]);
