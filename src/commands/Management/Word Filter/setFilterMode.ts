...
export default class extends SkyraCommand {
	...
	public async run(message: KlasaMessage, [type, mode = 'enable']: [string, string?]) {
		const level = message.guild!.settings.get(GuildSettings.Selfmod.Filter.SoftAction) as GuildSettings.Selfmod.Filter.SoftAction;
		if (type === 'show') {
		...
		await message.guild!.settings.update(GuildSettings.Selfmod.Filter.SoftAction, changed);
