...
@ApplyOptions<ModerationCommandOptions>({
	...
	description: (language) => language.get(LanguageKeys.Commands.Moderation.RestrictReactionDescription),
	extendedHelp: (language) => language.get(LanguageKeys.Commands.Moderation.RestrictReactionExtended),
...
export default class extends ModerationCommand {
	...
	public async inhibit(message: KlasaMessage) {
		...
		const { id, language } = await message.guild.readSettings((settings) => ({
			id: settings[GuildSettings.Roles.RestrictedReaction],
		...
		if (!role) {
			...
			if (await message.ask(language.get(LanguageKeys.Commands.Moderation.ActionSharedRoleSetupExisting))) {
				...
				await message.guild.writeSettings([[GuildSettings.Roles.RestrictedReaction, role.id]]);
