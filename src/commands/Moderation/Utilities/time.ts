...
export default class extends SkyraCommand {
	...
	private async checkBan(message: KlasaMessage, user: KlasaUser) {
		...
		const users = await message.guild.fetchBans().catch(() => { throw message.language.get('SYSTEM_FETCHBANS_FAIL'); });
	...
	private async checkVMute(message: KlasaMessage, user: KlasaUser) {
		...
		const member = await message.guild.members.fetch(user).catch(() => { throw message.language.get('USER_NOT_IN_GUILD'); });
