...
export default class extends ModerationCommand {
	...
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			description: language => language.tget('COMMAND_RESTRICTATTACHMENT_DESCRIPTION'),
			extendedHelp: language => language.tget('COMMAND_RESTRICTATTACHMENT_EXTENDED'),
	...
	public async inhibit(message: KlasaMessage) {
		...
		if (!role) {
			...
			if (await message.ask(message.language.tget('ACTION_SHARED_ROLE_SETUP_EXISTING'))) {
				...
				await message.guild.settings.update(GuildSettings.Roles.RestrictedAttachment, role, { throwOnError: true });
