...
export class UserPaginatedMessageCommand extends PaginatedMessageCommand {
	...
	private static generation = Args.make<number>((parameter, { argument }) => {
		...
		if (numberParameter && kPokemonGenerations.has(numberParameter)) return ok(numberParameter);
		return err(
			Args.error({ parameter, argument, identifier: LanguageKeys.Commands.Pokemon.LearnInvalidGeneration, context: { generation: parameter } })
		);
	});
}
