...
export class UserPaginatedMessageCommand extends PaginatedMessageCommand {
	...
	private static type = Args.make<Types[]>((parameter, { argument }) => {
		...
		if (lowerCasedSplitArguments.length > 2)
			return err(Args.error({ parameter, argument, identifier: LanguageKeys.Commands.Pokemon.TypeTooManyTypes }));

		for (const type of lowerCasedSplitArguments) {
			if (!kPokemonTypes.has(type))
				return err(Args.error({ parameter, argument, identifier: LanguageKeys.Commands.Pokemon.TypeNotAType, context: { type } }));
		}

		return ok(lowerCasedSplitArguments);
	});
}
