...
export default class extends SkyraCommand {
	...
	private async fetch(message: KlasaMessage, query: string): Promise<DefineLexicalEntries> {
		const data = await this.r.table(Databases.Oxford).get(query).run();
		if (data) {
		...
		try {
			...
			await this.r.table(Databases.Oxford).insert({ id: query, valid: true, lexicalEntries }).run();
			return lexicalEntries;
		} catch (error) {
			await this.r.table(Databases.Oxford).insert({ id: query, valid: false }).run();
