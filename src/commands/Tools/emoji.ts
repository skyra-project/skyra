...
export default class extends SkyraCommand {
	...
	public async run(message: KlasaMessage, [emoji]: [string]) {
		...
		if (!REG_TWEMOJI.test(emoji)) throw message.language.get(LanguageKeys.Commands.Tools.EmojiInvalid);
		const r = twemoji(emoji);
		const buffer = await fetch(`https://twemoji.maxcdn.com/72x72/${r}.png`, FetchResultTypes.Buffer).catch(() => {
			throw message.language.get(LanguageKeys.Commands.Tools.EmojiInvalid);
		});

		if (buffer.byteLength >= MAX_EMOJI_SIZE) throw message.language.get(LanguageKeys.Commands.Tools.EmojiTooLarge, { emoji });
