...
import { PermissionString, Permissions, TextChannel } from 'discord.js';
...
export default class extends Inhibitor {
	...
	public run(message: KlasaMessage, command: Command) {
		...
		if (missing.length) {
			...
			throw message.language.get('inhibitorMissingBotPerms', { missing: missing.map((permission) => permissions[permission]) });
		}
	}
}
