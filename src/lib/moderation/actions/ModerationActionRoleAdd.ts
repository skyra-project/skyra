...
export class ModerationActionRoleAdd extends ModerationAction<Role> {
	public constructor() {
		super({
	...
	protected override async handleApplyPre(guild: Guild, options: ModerationAction.Options, data: ModerationAction.Data<Role>) {
		...
		await api().guilds.addRoleToMember(guild.id, options.userId, role.id, { reason: await this.getReason(guild, options.reason) });
	...
	protected override async handleUndoPre(guild: Guild, options: ModerationAction.Options, data: ModerationAction.Data<Role>) {
		const role = data.context!;
		await api().guilds.removeRoleFromMember(guild.id, options.userId, role.id, { reason: options.reason ?? undefined });
