		...
		await queryRunner.createTable(
			new Table({
				...
				columns: [
					...
					new TableColumn({ 'name': 'access_token', 'type': 'char', 'length': '30', 'isNullable': true, 'default': null, 'comment': 'Token to access the dasboard' }),
					new TableColumn({ 'name': 'refresh_token', 'type': 'char', 'length': '30', 'isNullable': true, 'default': null, 'comment': 'Refresh token to refresh the access' }),
					new TableColumn({ 'name': 'expires_at', 'type': 'timestamp without time zone', 'isNullable': true, 'default': null, 'comment': 'Expiry timestamp for the token' })
				]
			})
		);
