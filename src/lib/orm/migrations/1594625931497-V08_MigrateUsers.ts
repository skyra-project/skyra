...
(
    id               varchar(19)                                         not null
        constraint users_user_idx
            primary key,
    command_uses     integer       default 0                             not null
        constraint users_command_uses_check
            check (command_uses >= 0),
    banner_list      varchar(6)[]  default '{}'::character varying(6)[]  not null,
    badge_set        varchar(6)[]  default '{}'::character varying(6)[]  not null,
    badge_list       varchar(6)[]  default '{}'::character varying(6)[]  not null,
    color            integer       default 0                             not null
        constraint users_color_check
            check ((color >= 0) AND (color <= 16777215)),
    marry            varchar(19)[] default '{}'::character varying(19)[] not null,
    money            bigint        default 0                             not null
        constraint users_money_check
            check (money >= 0),
    point_count      integer       default 0                             not null
        constraint users_point_count_check
            check (point_count >= 0),
    reputation_count integer       default 0                             not null
        constraint users_reputation_count_check
            check (reputation_count >= 0),
    theme_level      varchar(6)    default '1001'::character varying     not null,
    theme_profile    varchar(6)    default '0001'::character varying     not null,
    next_daily       bigint
        constraint users_next_daily_check
            check (next_daily >= 0),
    next_reputation  bigint
        constraint users_next_reputation_check
