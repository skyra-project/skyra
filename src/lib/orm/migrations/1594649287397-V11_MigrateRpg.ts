...
export class V11MigrateRpg1594649287397 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.createTable(new Table({
			...
			indices: [
				new TableIndex({ columnNames: ['name', 'rarity'], isUnique: true })
			],
			columns: [
				new TableColumn({ name: 'id', type: 'integer', isGenerated: true, isNullable: false, isPrimary: true }),
		...
		await queryRunner.createTable(new Table({
			name: 'rpg_user_item',
			checks: [
				new TableCheck({ expression: /* sql */`"durability" >= 0` })
			],
			columns: [
		...
		await queryRunner.createForeignKey('rpg_user_item', new TableForeignKey({
			columnNames: ['item_id'],
		...
		await queryRunner.createForeignKey('rpg_user', new TableForeignKey({
		...
		await queryRunner.createForeignKey('rpg_battle', new TableForeignKey({
