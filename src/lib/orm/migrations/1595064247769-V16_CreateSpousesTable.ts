...
export class V16CreateSpousesTable1595064247769 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.createTable(new Table({
			...
			foreignKeys: [
				new TableForeignKey({ columnNames: ['user_id_1'], referencedTableName: 'user', referencedColumnNames: ['id'], onDelete: 'CASCADE' }),
				new TableForeignKey({ columnNames: ['user_id_2'], referencedTableName: 'user', referencedColumnNames: ['id'], onDelete: 'CASCADE' })
			]
		}));
