...
export default Client.defaultGuildSchema
	...
	.add('notifications', (folder) =>
		folder.add('streams', (streams) =>
			streams.add('twitch', (twitch) => twitch.add('streamers', 'TwitchSubscription', { array: true, configurable: false }))
		)
	)
	.add('suggestions', (folder) =>
		folder
			.add('channel', 'TextChannel')
