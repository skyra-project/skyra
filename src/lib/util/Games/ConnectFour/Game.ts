...
export class Game {
	...
	public async run() {
		this.message = await this.message.send(this.language.get('SYSTEM_LOADING'));
		for (const reaction of ConnectFourConstants.Reactions) await this.message.react(reaction);
		this.content = this.language.get('COMMAND_C4_GAME_NEXT', { user: this.next!.name, turn: this.next!.color });
	...
	private async updateContent() {
		try {
			...
			if (error instanceof DiscordAPIError && (error.code === APIErrors.UnknownChannel || error.code === APIErrors.UnknownMessage)) {
				if (error.code !== APIErrors.UnknownChannel) await this.message.alert(this.message.language.get('COMMAND_C4_GAME_DRAW'));
				this.stop();
			} else {
