...
export async function fetchSaelem<R extends SaelemQueryReturnTypes>(query: string, variables: SaelemQueryVariables<R>) {
	try {
		return await fetch<SaelemResponse<R>>(SAELEM_GRAPHQL_API_URL, {
...
type SaelemQueryVariables<R extends SaelemQueryReturnTypes> =
	R extends 'getHoroscope'
		? QueryGetHoroscopeArgs
		: never;
