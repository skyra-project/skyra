...
export async function handleCommandError(error: unknown, payload: MessageCommandErrorPayload | MessageSubcommandErrorPayload) {
	...
	try {
		await sendTemporaryMessage(message, generateUnexpectedErrorMessage(message, command, t, error));
	} catch (err) {
...
function generateUnexpectedErrorMessage(message: Message, command: Command, t: TFunction, error: Error) {
