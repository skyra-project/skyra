...
@ApplyOptions<RouteOptions>({ name: 'webhooks/discords', route: 'webhooks/discords' })
export class UserRoute extends Route {
	public async [methods.POST](request: ApiRequest, response: ApiResponse) {
		if (request.headers.authorization !== process.env.WEBHOOK_DISCORDS_TOKEN) return response.forbidden();
