...
export class UserModerationTask extends ModerationTask {
	protected async handle(guild: Guild, data: ModerationData) {
		...
		const reason = `[MODERATION] Emoji Restricted released after ${t(LanguageKeys.Globals.DurationValue, { value: data.duration })}`;
		const actionData = await this.getActionData(guild, data.userID);
		await ModerationActions.restrictedEmoji.undo(guild, { user: data.userID, reason }, actionData);
		return null;
	}
}
