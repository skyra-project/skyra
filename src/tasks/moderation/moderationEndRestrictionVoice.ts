...
export class UserModerationTask extends ModerationTask {
	protected async handle(guild: Guild, data: ModerationData) {
		...
		const reason = `[MODERATION] Voice Restricted released after ${t(LanguageKeys.Globals.DurationValue, { value: data.duration })}`;
		const actionData = await this.getActionData(guild, data.userID);
		await ModerationActions.restrictedVoice.undo(guild, { userId: data.userID, reason }, actionData);
		return null;
	}
}
