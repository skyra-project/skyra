...
export class UserModerationTask extends ModerationTask {
	protected async handle(guild: Guild, data: ModerationData) {
		...
		const reason = `[MODERATION] Warning released after ${t(LanguageKeys.Globals.DurationValue, { value: data.duration })}`;
		const actionData = await this.getActionData(guild, data.userID, data.caseID);
		await ModerationActions.warning.undo(guild, { userId: data.userID, reason }, actionData);
		return null;
	}
}
