...
describe('WorkerManager', () => {
	...
	test('GIVEN new instance THEN has initial data', async () => {
		...
		const start0 = vi.spyOn(handler.workers[0], 'start' as any);
		const start1 = vi.spyOn(handler.workers[1], 'start' as any);

		const destroy0 = vi.spyOn(handler.workers[0], 'destroy' as any);
		const destroy1 = vi.spyOn(handler.workers[1], 'destroy' as any);
